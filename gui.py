...
def build_timetable(timetable):
    ...
    for i in range(len(timetable)):
        ...
        for j in range(len(timetable[0])):
            ...
            if timeslot:
                html += "<td class='{}'>{}</td>".format(timeslot[0], timeslot[1])
            else:
...
def build_output_gui(timetable):
    html = begin_document("EXAMPLE TITLE")
    html += build_timetable(timetable)
    html += end_document()

    with open("output/file.html", "w", encoding="utf-8") as file:
        file.write(html)
