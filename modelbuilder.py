...
def build():
    ...
    for c_in in imported_courses:
        ...
        if(len(c_in[2]) > 1):
            a = mdl.integer_var(name=c_in[2].pop(
                0), domain=range(TIMESLOTS))
            model.all_activities.append(a)
            model.flex.append(a)
            c.flex.append(a)
            prev_a = a
            for fl in c_in[2]:
                a = mdl.integer_var(name=fl, domain=range(TIMESLOTS))
                model.all_activities.append(a)
                model.flex.append(a)
                c.flex.append(a)
                # break symmetry
    ...
    if imported_options.remove_weekend == True:
        blocked_hours = get_timeslots_of_day(
            list(range(TIMESLOTS)), SAT) + get_timeslots_of_day(list(range(TIMESLOTS)), SUN)
        for h in blocked_hours:
            if h not in domain:
                continue
            a = mdl.integer_var(name="weekend")
            mdl.add(a == h)
            model.all_activities.append(a)
    ...
    if(s0 != s1):
        blocked_hours = get_timeslots_in_interval(list(range(TIMESLOTS)), s0, s1)
        for h in blocked_hours:
            if h not in domain:
                continue
            a = mdl.integer_var(name="sleep")
            mdl.add(a == h)
            model.all_activities.append(a)
            model.blocked.append(a)
            domain.remove(h)
    ...
    if(len(model.all_activities) < TIMESLOTS):
        ...
        for i in range(1, 1 + TIMESLOTS - len(model.all_activities)):
            a = mdl.integer_var(name=(f"free activity: {i}"), domain=domain)
            model.all_activities.append(a)
            model.free.append(a)
            # break symmetry
            mdl.add(prev_a < a)
    ...
    for i in range(TIMESLOTS):
        model.all_timeslots.append(mdl.integer_var(
            name=timeslot_to_string(i), domain=range(len(model.all_activities))))
